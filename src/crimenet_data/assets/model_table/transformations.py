...
def split_expression(
    ...
    return (
        pl.when(timestamp < pl.lit(TRAIN_END_UTC))
        .then(pl.lit("train"))
        .when(timestamp < pl.lit(VALIDATION_END_UTC))
...
def prepare_observed_rows(
    ...
    return (
        ...
        .with_columns(
            ...
            pl.lit("observed_event").alias("row_type"),
            ...
            pl.lit(
                0.0,
                dtype=pl.Float64,
            ).alias("integration_weight_cell_seconds"),
            ...
            .alias("row_timestamp_utc"),
        )
        .with_columns(
            split_expression("row_timestamp_utc").alias("split"),
            pl.col("row_timestamp_utc").dt.year().cast(pl.Int32).alias("row_year"),
        )
        .select(
            "model_row_id",
            "row_type",
...
def prepare_integration_rows(
    ...
    return (
        integration.with_columns(
            pl.concat_str(
                [
                    pl.lit("integration"),
                    pl.col("integration_sample_id"),
                ],
                separator="|",
            ).alias("model_row_id"),
            pl.lit("integration_sample").alias("row_type"),
            pl.lit(False).alias("is_observed_event"),
            pl.lit(
                0,
                dtype=pl.Int8,
            ).alias("event_count"),
            pl.col("sample_timestamp_utc")
            .cast(
                pl.Datetime(
                    "us",
                    time_zone="UTC",
            ...
            pl.lit(
                None,
                dtype=pl.String,
            ).alias("canonical_family_code"),
            pl.lit(
                None,
                dtype=pl.String,
            ).alias("canonical_offense_family"),
            pl.lit(
                None,
                dtype=pl.String,
            ).alias("canonical_subtype_code"),
            pl.lit(
                None,
                dtype=pl.String,
            ).alias("canonical_offense_subtype"),
        )
        .with_columns(
            split_expression("row_timestamp_utc").alias("split"),
            pl.col("row_timestamp_utc").dt.year().cast(pl.Int32).alias("row_year"),
...
def build_query_rows(
    *,
    ...
    return pl.concat(
        [
            prepare_observed_rows(events),
            prepare_integration_rows(integration),
        ],
...
def build_cell_prefix(
    ...
    return (
        ...
        .with_columns(
            pl.col("_delta_all").cum_sum().over(groups).alias("_cum_all"),
            pl.col("_delta_violent").cum_sum().over(groups).alias("_cum_violent"),
            pl.col("_delta_property").cum_sum().over(groups).alias("_cum_property"),
        )
    )
...
def build_city_prefix(
    ...
    return (
        ...
        .agg(pl.len().cast(pl.Int64).alias("_delta_all"))
        .sort(
            [
                *groups,
                "occurrence_timestamp_utc",
        ...
        .with_columns(pl.col("_delta_all").cum_sum().over(groups).alias("_cum_all"))
    )
    ...
    return (
        ...
        .with_columns(
            plh3.grid_disk(
                "osm_h3_cell_id",
                1,
            ).alias("_target_h3_cells")
        )
        .explode("_target_h3_cells")
        .select(
            "source_city",
            "occurrence_timestamp_utc",
            pl.col("_target_h3_cells").cast(pl.Int64).alias("osm_h3_cell_id"),
        )
        .group_by(
            [
                *groups,
...
def prefix_lookup(
    *,
    ...
    if include_pulse_time:
        selected.append(pl.col("_pulse_time").alias(f"_last_occurrence_at_utc{suffix}"))
...
def attach_prefix_history(
    *,
    ...
    cumulative_columns = list(cumulative_map)

    end = prefix_lookup(
        rows=rows,
        prefix=prefix,
        by=by,
        query_time=pl.col("row_timestamp_utc"),
        cumulative_columns=cumulative_columns,
        suffix="_end",
        include_pulse_time=add_recency,
    )
    ...
    for (
        window_name,
        seconds,
    ) in HISTORY_WINDOWS_SECONDS.items():
        lower = prefix_lookup(
            rows=rows,
            prefix=prefix,
            by=by,
            query_time=(pl.col("row_timestamp_utc") - pl.duration(seconds=seconds)),
            cumulative_columns=cumulative_columns,
            suffix=f"_lower_{window_name}",
        ...
        for (
            ...
            expressions.append(
                (
                    pl.col(f"{cumulative_column}_end").fill_null(0)
                    - pl.col(f"{cumulative_column}_lower_{window_name}").fill_null(0)
                )
                .clip(lower_bound=0)
                .cast(pl.Int64)
                .alias(f"{output_prefix}_{output_base}_{window_name}")
            )

        result = result.with_columns(expressions)

    if add_recency:
        last_column = "_last_occurrence_at_utc_end"
        ...
        result = result.with_columns(
            ...
            ).alias(f"has_{output_prefix}_28d"),
            pl.when(pl.col(last_column).is_not_null())
            .then(
                (
                    (pl.col("row_timestamp_utc") - pl.col(last_column))
                    .dt.total_seconds()
                    .cast(pl.Float64)
                    / 3600.0
            ...
            .alias(f"hours_since_last_{output_prefix}_capped_28d"),
        )

    temporary_columns = [
        column
        for column in result.collect_schema().names()
        if column.startswith(("_cum_", "_delta_"))
    ]
...
def attach_dynamic_history(
    *,
    ...
    cell_prefix = build_cell_prefix(history_events)

    city_prefix = build_city_prefix(history_events)

    k1_prefix = build_k1_prefix(history_events)
    ...
    result = result.rename(
        {
            "has_cell_28d": "has_crime_cell_28d",
            "hours_since_last_cell_capped_28d": "hours_since_last_crime_cell_capped_28d",
        }
    )
    ...
    result = result.rename(
        {
            "has_city_28d": "has_crime_city_28d",
            "hours_since_last_city_capped_28d": "hours_since_last_crime_city_capped_28d",
        }
    )
...
def local_hour_expression() -> pl.Expr:
    ...
    for city, timezone in reversed(list(CITY_TIMEZONES.items())):
        result = (
            pl.when(pl.col("source_city") == city)
            .then(pl.col("row_timestamp_utc").dt.convert_time_zone(timezone).dt.hour())
            .otherwise(result)
        )

    return result.alias("local_hour")
...
def local_weekday_expression() -> pl.Expr:
    ...
    return result.alias("local_day_of_week")
...
def add_calendar_features(
    ...
    return rows.with_columns(
        (pl.col("local_hour").cast(pl.Float64) * (2.0 * math.pi / 24.0))
        .sin()
        .alias("local_hour_sin"),
        (pl.col("local_hour").cast(pl.Float64) * (2.0 * math.pi / 24.0))
        .cos()
        .alias("local_hour_cos"),
        (pl.col("local_day_of_week").cast(pl.Float64) * (2.0 * math.pi / 7.0))
        .sin()
...
def validate_final_partition(
    ...
    *,
    ...
    history_audit_columns = [
        column
        for column in rows.collect_schema().names()
        if column.startswith("_last_occurrence_at_utc")
    ]

    leak_expressions = []

    for column in history_audit_columns:
        leak_expressions.append(
            (
                pl.col(column).is_not_null()
                & (pl.col(column) >= pl.col("row_timestamp_utc"))
            )
            .sum()
            .cast(pl.Int64)
            .alias(f"leak__{column}")
        )

    negative_history = [
        column
        for column in rows.collect_schema().names()
        if ("_count_" in column and column.startswith(("cell_", "city_", "k1_")))
    ]

    metrics = (
        rows.select(
            pl.len().cast(pl.Int64).alias("rows"),
            pl.col("model_row_id").n_unique().cast(pl.Int64).alias("unique_row_ids"),
            pl.col("_lighting_matched")
            .fill_null(False)
            .not_()
            .sum()
            .cast(pl.Int64)
            .alias("missing_lighting"),
            (pl.col("split") != split_expression("row_timestamp_utc"))
            .sum()
            .cast(pl.Int64)
            .alias("split_mismatches"),
            (pl.col("is_observed_event") & (pl.col("event_count") != 1))
            .sum()
            .cast(pl.Int64)
            .alias("bad_observed_event_count"),
            (~pl.col("is_observed_event") & (pl.col("event_count") != 0))
            .sum()
            .cast(pl.Int64)
            .alias("bad_integration_event_count"),
            (
                pl.col("is_observed_event")
                & (pl.col("integration_weight_cell_seconds") != 0.0)
            )
            .sum()
            .cast(pl.Int64)
            .alias("bad_observed_weight"),
            (
                ~pl.col("is_observed_event")
                & (pl.col("integration_weight_cell_seconds") <= 0.0)
            )
            .sum()
            .cast(pl.Int64)
            .alias("bad_integration_weight"),
            *[
                (pl.col(column) < 0).sum().cast(pl.Int64).alias(f"negative__{column}")
                for column in negative_history
            ],
            *leak_expressions,
        )
        .collect()
    ...
    fatal = {
        ...
        if (key != "rows" and key != "unique_row_ids" and value != 0)
    }

    if metrics["unique_row_ids"] != metrics["rows"]:
        fatal["duplicate_model_row_ids"] = metrics["rows"] - metrics["unique_row_ids"]

    if fatal:
        raise ValueError(f"Final model-table validation failed: {fatal}")
