...
6. frames / upload-preprocessed
   ...
   - optionally upload the prepared cache + compact handoff metadata to B2 so the
     CPU/NVMe host can terminate before any GPU rental begins.
7. fetch-preprocessed / embed
   - on a separate GPU host, download only the prepared Parquet cache; no raw TIFFs,
     Rasterio work, scene selection, or existing Gold embedding download is needed.
...
def make_paths(work_dir: str) -> Paths:
    ...
    return Paths(
        ...
        handoff=ensure_dir(work / "handoff"),
...
# -----------------------------------------------------------------------------
# CPU -> GPU prepared-cache handoff via B2
# -----------------------------------------------------------------------------

PREPARED_CACHE_FORMAT = "crimenet-sentinel-prepared-v1"
PREPARED_CACHE_MANIFEST = "preprocess_handoff.json"


def remote_join(root: str, *parts: str) -> str:
    """Join paths for rclone remotes without damaging `remote:` prefixes."""
    base = root.rstrip("/")
    suffix = "/".join(str(x).strip("/") for x in parts if str(x).strip("/"))
    return f"{base}/{suffix}" if suffix else base


def existing_gold_schema(paths: Paths) -> dict[str, str] | None:
    files = sorted(paths.existing_gold.glob("part-*.parquet"))
    if not files:
        return None
...
def frame_cache_stats(paths: Paths) -> dict[str, Any]:
    files = sorted(paths.frames.glob("bucket=*/part-*.parquet"))
    rows = 0
    total_bytes = 0
    entries: list[dict[str, Any]] = []
    for path in files:
        rel = path.relative_to(paths.frames).as_posix()
        size = path.stat().st_size
        count = pq.ParquetFile(path).metadata.num_rows
        rows += int(count)
        total_bytes += int(size)
        entries.append({"path": rel, "bytes": int(size), "rows": int(count)})
    return {
        "frames": rows,
        "shards": len(files),
...
def build_preprocess_handoff(args: argparse.Namespace, paths: Paths) -> dict[str, Any]:
    frames_done = paths.manifests / "frames.done.json"
    if not frames_done.exists():
        die("Prepared-frame cache is incomplete: frames.done.json is missing")
    recorded = json.loads(frames_done.read_text())
    actual = frame_cache_stats(paths)
    expected_frames = int(recorded.get("frames", recorded.get("rows", 0)))
    expected_shards = int(recorded.get("shards", 0))
    if expected_frames != int(actual["frames"]) or expected_shards != int(actual["shards"]):
        die(
            "Prepared-frame cache does not match frames.done.json: "
            f"recorded frames/shards={expected_frames}/{expected_shards}, "
            f"actual={actual['frames']}/{actual['shards']}"
        )

    patch_src = paths.selection / "existing_validity_patch.parquet"
    patch_name = None
    if patch_src.exists():
        patch_name = "existing_validity_patch.parquet"
        shutil.copy2(patch_src, paths.handoff / patch_name)

    payload = {
        "format": PREPARED_CACHE_FORMAT,
        "created_at_utc": iso_z(utcnow()),
        "scientific_contract": {
            ...
            "prepared_spectral_dtype": "uint16",
            "prepared_scl_dtype": "uint8",
            "prepared_spectral_shape": list(SPECTRAL_DN_SHAPE),
            "prepared_scl_shape": list(SCL_SHAPE),
        },
        "frame_buckets": int(args.frame_buckets),
        "frames": int(actual["frames"]),
        "shards": int(actual["shards"]),
        "parquet_bytes": int(actual["parquet_bytes"]),
        "files": actual["files"],
        "existing_gold": {
            "remote": None if args.no_existing_gold else args.existing_gold_remote,
            "part_start": existing_shard_count(paths),
            "schema": existing_gold_schema(paths),
        },
        "validity_patch": patch_name,
        "source": {
            "b2_source_remote": args.b2_source_remote,
            "existing_gold_remote": None if args.no_existing_gold else args.existing_gold_remote,
        },
    }
    out = paths.handoff / PREPARED_CACHE_MANIFEST
...
def rclone_copy_dir(
    ...
    *,
    ...
    rclone_exists()
    operation = "sync" if sync_remote else "copy"
    run([
        "rclone", operation, str(local), remote,
        "--fast-list",
        "--transfers", str(args.cache_transfers),
        "--checkers", str(args.cache_checkers),
    ...
    if verify:
        run([
            "rclone", "check", str(local), remote,
            "--one-way", "--size-only",
            "--checkers", str(args.cache_checkers),
        ])


def phase_upload_preprocessed(args: argparse.Namespace, paths: Paths) -> None:
    if not args.preprocessed_remote:
        die("--preprocessed-remote must be set for upload-preprocessed")
    payload = build_preprocess_handoff(args, paths)
    frames_remote = remote_join(args.preprocessed_remote, "frames")
    handoff_remote = remote_join(args.preprocessed_remote, "handoff")
    log(
        f"upload-preprocessed: frames={payload['frames']:,}, shards={payload['shards']:,}, "
        f"parquet={payload['parquet_bytes']/1024**3:.1f} GiB -> {args.preprocessed_remote}"
    )
    # This is a dedicated temporary cache prefix. Use sync so a rerun cannot leave
    # stale frame shards from an older preprocessing attempt on B2.
    rclone_copy_dir(
        args, paths.frames, frames_remote,
        verify=args.verify_cache_transfer, sync_remote=True,
    )
    rclone_copy_dir(
        args, paths.handoff, handoff_remote,
        verify=args.verify_cache_transfer, sync_remote=True,
    )
    done = paths.manifests / "upload_preprocessed.done.json"
    done.write_text(json.dumps({
        "created_at_utc": iso_z(utcnow()),
        "remote": args.preprocessed_remote,
        "frames": payload["frames"],
        "shards": payload["shards"],
        "parquet_bytes": payload["parquet_bytes"],
    }, indent=2))
    log(f"upload-preprocessed complete -> {args.preprocessed_remote}")


def validate_downloaded_preprocessed(args: argparse.Namespace, paths: Paths) -> dict[str, Any]:
    manifest_path = paths.handoff / PREPARED_CACHE_MANIFEST
    if not manifest_path.exists():
        die(f"Downloaded prepared cache is missing {PREPARED_CACHE_MANIFEST}")
    payload = json.loads(manifest_path.read_text())
    if payload.get("format") != PREPARED_CACHE_FORMAT:
        die(
            f"Unsupported prepared-cache format {payload.get('format')!r}; "
            f"expected {PREPARED_CACHE_FORMAT!r}"
        )
    expected_buckets = int(payload["frame_buckets"])
    if int(args.frame_buckets) != expected_buckets:
        die(
            f"--frame-buckets={args.frame_buckets} disagrees with prepared cache "
            f"({expected_buckets}). Use the cache's bucket count."
        )
    ...
    for entry in expected_files:
        path = paths.frames / str(entry["path"])
        if not path.exists():
        ...
        if got != want:
            wrong_size.append((str(entry["path"]), want, got))
    if missing or wrong_size:
        die(
            f"Prepared cache validation failed: missing={missing[:10]}, "
            f"wrong_size={wrong_size[:10]}"
        )

    actual = frame_cache_stats(paths)
    if int(actual["frames"]) != int(payload["frames"]) or int(actual["shards"]) != int(payload["shards"]):
        die(
            "Prepared cache row/shard mismatch after download: "
            f"expected={payload['frames']}/{payload['shards']} "
            f"actual={actual['frames']}/{actual['shards']}"
        )

    patch_name = payload.get("validity_patch")
    if patch_name:
        src = paths.handoff / str(patch_name)
        if not src.exists():
            die(f"Prepared handoff references missing validity patch {patch_name}")
        shutil.copy2(src, paths.selection / "existing_validity_patch.parquet")

    # Give downstream manifests/finalization the same frame completion record even
    # though this host did not build the frames itself.
    (paths.manifests / "frames.done.json").write_text(json.dumps({
        "created_at_utc": payload.get("created_at_utc"),
        "frames": payload["frames"],
        "buckets": payload["frame_buckets"],
        "shards": payload["shards"],
        "parquet_bytes": payload["parquet_bytes"],
        "source": "downloaded_preprocessed_cache",
        "remote": args.preprocessed_remote,
    }, indent=2))
    return payload


def phase_fetch_preprocessed(args: argparse.Namespace, paths: Paths) -> None:
    if not args.preprocessed_remote:
        die("--preprocessed-remote must be set for fetch-preprocessed")
    done = paths.manifests / "fetch_preprocessed.done.json"
    if args.resume and done.exists():
        # Revalidate local files before trusting the resume marker.
        payload = validate_downloaded_preprocessed(args, paths)
        log(
            f"fetch-preprocessed: local cache already complete; "
            f"frames={payload['frames']:,}, shards={payload['shards']:,}"
        )
        return

    rclone_exists()
    ensure_dir(paths.frames)
    ensure_dir(paths.handoff)
    log(f"fetch-preprocessed: {args.preprocessed_remote} -> {paths.work}")
    stage_remote(
        remote_join(args.preprocessed_remote, "handoff"),
        paths.handoff,
        transfers=max(4, min(args.cache_transfers, 16)),
        checkers=max(8, min(args.cache_checkers, 32)),
        multi_thread_streams=max(2, min(args.rclone_multi_thread_streams, 8)),
    )
    stage_remote(
        remote_join(args.preprocessed_remote, "frames"),
        paths.frames,
        transfers=args.cache_transfers,
        checkers=args.cache_checkers,
        multi_thread_streams=args.rclone_multi_thread_streams,
    )
    payload = validate_downloaded_preprocessed(args, paths)
    done.write_text(json.dumps({
    ...
    log(
        f"fetch-preprocessed complete: frames={payload['frames']:,}, "
        f"parquet={payload['parquet_bytes']/1024**3:.1f} GiB"
    )
...
def phase_finalize(args: argparse.Namespace, paths: Paths) -> None:
    ...
    handoff_manifest = paths.handoff / PREPARED_CACHE_MANIFEST
    if handoff_manifest.exists():
        handoff_payload = json.loads(handoff_manifest.read_text())

    if args.part_start >= 0:
        start_idx = args.part_start
    elif sorted(paths.existing_gold.glob("part-*.parquet")):
        start_idx = existing_shard_count(paths)
    else:
        start_idx = int(handoff_payload.get("existing_gold", {}).get("part_start", 0))
    ...
    if new_files:
        ...
        new_schema_dict = {k: str(v) for k, v in new_schema.items()}
        if existing_files:
            old_schema = pl.read_parquet_schema(existing_files[0])
            old_schema_dict = {k: str(v) for k, v in old_schema.items()}
        else:
            old_schema_dict = handoff_payload.get("existing_gold", {}).get("schema")
        if old_schema_dict:
            schema_audit = {
                "passed": old_schema_dict == new_schema_dict,
                "old": old_schema_dict,
                "new": new_schema_dict,
                "source": "local_existing_gold" if existing_files else "preprocess_handoff",
            }
    ...
    recipe = {
        ...
        "execution": {
            ...
            "prepared_cache_remote": args.preprocessed_remote,
            "split_cpu_gpu_execution": bool(handoff_payload),
            "multi_gpu": True,
            "frame_buckets": args.frame_buckets,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        choices=[
            "all", "preprocess", "gpu",
            "stage", "inventory", "candidates", "select", "context", "frames",
            "upload-preprocessed", "fetch-preprocessed",
            "embed", "finalize", "publish",
        ],
        default="all",
    )
    ...
    p.add_argument(
        "--preprocessed-remote",
        default="b2:crimenet-data/tmp/sentinel_prepared_v1",
        help="Temporary durable B2 cache used to hand prepared frames from CPU hosts to GPU hosts",
    )
    ...
    # Prepared-cache CPU -> GPU transfer.
    p.add_argument("--cache-transfers", type=int, default=64)
    p.add_argument("--cache-checkers", type=int, default=128)
    p.add_argument("--verify-cache-transfer", action=argparse.BooleanOptionalAction, default=True)
...
def validate_args(args: argparse.Namespace) -> None:
    positive = {
        ...
        "cache_checkers": args.cache_checkers,
        "gpu_batch_size": args.gpu_batch_size,
        "gpu_prep_threads": args.gpu_prep_threads,
        "rows_per_output_shard": args.rows_per_output_shard,
...
def main() -> int:
    ...
    if args.phase == "all":
        ...
        phases = ["fetch-preprocessed", "embed", "finalize", "publish"]
    else:
        phases = [args.phase]
    ...
    dispatch = {
        ...
        "frames": phase_frames,
        "upload-preprocessed": phase_upload_preprocessed,
        "fetch-preprocessed": phase_fetch_preprocessed,
        "embed": phase_embed,
