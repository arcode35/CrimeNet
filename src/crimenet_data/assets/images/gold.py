...
class ImageryEmbeddingConfig(dg.Config):
    ...
    naip_batch_size: int = 16
    naip_prefetch_batches: int = 16
    naip_reader_workers: int = 8
    naip_read_chunk_size: int = 128
...
def _ensure_gdal_gcs_credentials() -> None:
    ...
    # On Compute Engine, GDAL /vsigs/ can use the VM's attached service
    # account through the metadata service. CPL_MACHINE_IS_GCE avoids relying
    # on heuristic detection inside GDAL.
    is_gce = bool(os.getenv("GCE_METADATA_HOST"))
    if not is_gce:
        product_name = Path("/sys/class/dmi/id/product_name")
        try:
            is_gce = product_name.exists() and "Google" in product_name.read_text()
        except OSError:
            is_gce = False

    if is_gce:
        os.environ.setdefault("CPL_MACHINE_IS_GCE", "YES")
        return

    raise RuntimeError(
        "GDAL /vsigs/ cannot find GCS credentials. Use a GCE attached service "
        "account, run `gcloud auth application-default login`, or set "
...
def _raster_env() -> rasterio.Env:
    ...
    if os.getenv("CPL_MACHINE_IS_GCE", "").upper() == "YES":
        options["CPL_MACHINE_IS_GCE"] = "YES"
...
def _olmo_timestamps(rows: list[dict]) -> np.ndarray:
    # OlmoEarth timestamps are [day, month_index_0_to_11, year].
    result = np.zeros((len(rows), 3), dtype=np.int64)
    for i, row in enumerate(rows):
        dt = row["capture_timestamp_utc"]
        result[i] = [dt.day, dt.month - 1, dt.year]
    return result
...
def _encode_naip(
    *,
    ...
    if config.naip_batch_size < 1:
        raise ValueError("naip_batch_size must be >= 1")
    if config.naip_reader_workers < 1:
        raise ValueError("naip_reader_workers must be >= 1")
    if config.naip_read_chunk_size < 1:
        raise ValueError("naip_read_chunk_size must be >= 1")
    if config.naip_prefetch_batches < 1:
        raise ValueError("naip_prefetch_batches must be >= 1")
    ...
    context.log.info(
        "NAIP parallel pipeline: "
        f"readers={config.naip_reader_workers}, "
        f"read_chunk={config.naip_read_chunk_size}, "
        f"prefetch={config.naip_prefetch_batches}, "
        f"gpu_batch={config.naip_batch_size}"
    )

    # Chunks never span GCS objects. A worker owns its DatasetReader for the
    # complete task, avoiding shared GDAL handles while amortizing COG opens.
    def iter_uri_chunks():
        current_uri = None
        chunk: list[dict] = []
        for row in work_df.iter_rows(named=True):
            uri = row["gcs_uri"]
            if chunk and (uri != current_uri or len(chunk) >= config.naip_read_chunk_size):
                yield chunk
                chunk = []
            if not chunk:
                current_uri = uri
    ...
    def read_chunk(rows: list[dict]) -> list[np.ndarray]:
        if not rows:
            return []
        uri = rows[0]["gcs_uri"]
        if any(r["gcs_uri"] != uri for r in rows):
            raise RuntimeError("NAIP read chunk unexpectedly spans multiple GCS objects")
        with _raster_env():
            with rasterio.open(gcs_uri_to_vsigs(uri), sharing=False) as src:
                return [
                    _naip_rgb_uint8(src, row, config.naip_image_size)
                    for row in rows
                ]

    effective_batch_size = config.naip_batch_size
    run_started = time.monotonic()

    def encode_ready(rows: list[dict], images: list[np.ndarray]) -> None:
        nonlocal shard_rows, shard_idx, encoded, effective_batch_size
        if len(rows) != len(images):
            raise RuntimeError(f"NAIP row/image mismatch: {len(rows)} != {len(images)}")

        offset = 0
        while offset < len(rows):
            take = min(effective_batch_size, len(rows) - offset)
            sub_rows = rows[offset:offset + take]
            sub_images = images[offset:offset + take]
            try:
                ...
                context.log.warning(
                    f"CUDA OOM in DINO; automatically reducing NAIP batch "
                    f"{old_batch} -> {effective_batch_size}"
                )
                continue

            previous_encoded = encoded
            for row, z in zip(sub_rows, embeddings, strict=True):
                shard_rows.append(
            ...
            if encoded // 5000 > previous_encoded // 5000:
                ...
                context.log.info(
                    f"DINOv3 NAIP encoded: {encoded:,}/{df.height:,} "
                    f"({rate:.2f} rows/s, gpu_batch={effective_batch_size})"
                )

    chunks = iter(iter_uri_chunks())
    max_pending = max(config.naip_reader_workers, config.naip_prefetch_batches)
    pending = deque()

    try:
        with ThreadPoolExecutor(
            max_workers=config.naip_reader_workers,
            ...
            def submit_next() -> bool:
                try:
                    ...
                    return False
                pending.append((rows, pool.submit(read_chunk, rows)))
                return True
            ...
            while pending:
                # Consume in submission order so shard/resume semantics remain
                # identical to the deterministic dataframe order.
                rows, future = pending.popleft()
                images = future.result()
                submit_next()  # refill before GPU work to maximize overlap
                encode_ready(rows, images)
