...
def enumerate_recent_us_scenes(
    ...
    states = load_state_geometries(work_dir, include_territories)
...
def process_asset(task: AssetTask) -> dict[str, Any]:
    ...
    s3 = make_s3_client()
    remote_size = object_size(s3, task.bucket, key)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Seed recent nationwide Sentinel-2 L2A imagery into Backblaze B2."
    )
...
def main() -> int:
    ...
    all_scenes = enumerate_recent_us_scenes(
        work_dir,
        start,
        end,
        args.include_territories,
    )
    log(f"Unique recent U.S.-intersecting scenes: {len(all_scenes):,}")
    ...
    manifest = {
        ...
        "scenes": [scene_manifest_entry(s, args.prefix) for s in selected],
    }

    local_manifest = work_dir / "national_sentinel_selection.json"
    write_local_manifest(local_manifest, manifest)

    manifest_key = (
        f"{args.prefix.strip('/')}/manifests/"
        f"selection_{start.date().isoformat()}_{end.date().isoformat()}.json"
    )
    put_json_verified(s3, args.bucket, manifest_key, manifest)
    log(f"manifest -> b2://{args.bucket}/{manifest_key}")

    if args.dry_run:
        log("DRY RUN complete; no imagery assets copied.")
        return 0
    ...
    completion_key = (
        f"{args.prefix.strip('/')}/manifests/"
        f"completion_{start.date().isoformat()}_{end.date().isoformat()}.json"
    )
    ...
    success_key = f"{args.prefix.strip('/')}/_BOOTSTRAP_SUCCESS.json"
    put_json_verified(
        s3,
        args.bucket,
        success_key,
